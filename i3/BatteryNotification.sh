...
if [[ `echo $(/usr/bin/acpi -b) | grep Discharging` &&  `echo $BATTINFO | cut -f 5 -d " "` < 00:15:00 ]] ; then
...
if [[ `echo $(/usr/bin/acpi -b) | grep Discharging` && `echo $BATTINFO | cut -f 5 -d " "` <00:05:00 ]] ; then
